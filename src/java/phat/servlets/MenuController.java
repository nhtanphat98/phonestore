...
public class MenuController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            if(result == null){
                result = "SignIn";
                session.setAttribute("avatar", "unknow.png");
                session.setAttribute("result", result);
            }
            if(listCategory == null){
                CategoryDAO cateDAO = new CategoryDAO();
                listCategory = cateDAO.getAllCategory();
                ProductDAO productDAO = new ProductDAO();
                for (CategoryDTO categoryDTO : listCategory) {
                    List<ProductDTO> listProduct = productDAO.getAllProductByCateID(categoryDTO.getCateID());
                    categoryDTO.setListProduct(listProduct);
                }
